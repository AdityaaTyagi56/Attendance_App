...
const App: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background text-primary">
        ...
        >
          ...
          <p className="text-green-500 transition-opacity duration-200">{connectionStatus}</p>
        </motion.div>
      </div>
    );
  }
  ...
  }, [navigationItems, currentView]);

  const NavLinks = React.useMemo(() => (
    <>
      {navigationItems.map(item => (
        <button key={item.view} onClick={() => handleViewChange(item.view)}
          className={`relative w-full text-left flex items-center p-3 rounded-2xl font-medium transition-fast text-sm
        ${currentView === item.view
          ...
          <item.icon className={`w-5 h-5 mr-3 ${currentView === item.view ? 'text-primary' : 'text-text-secondary'}`} />
          {item.label}
        </button>
      ))}
    </>
  ), [navigationItems, currentView, handleViewChange]);

  const ThemeToggle = React.useMemo(() => (
    <button onClick={toggleTheme} className={`w-full text-left flex items-center p-3 rounded-2xl text-text-secondary hover:bg-surface/50 hover:text-text font-medium transition-fast text-sm`}>
      {theme === 'light' ?
    ...
    <div className="min-h-screen bg-background text-text transition-colors duration-300">
      {isMobileMenuVisible && (
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          />
          <div
            className="fixed inset-y-0 left-0 w-72 glass text-text flex flex-col z-50 border-r border-white/10 transform transition-transform duration-200 ease-out"
