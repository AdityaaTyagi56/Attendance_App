...
async function callBackend(endpoint: string, data: any) {
  try {
    console.log(`[GeminiService] Calling ${API_BASE_URL}${endpoint}`);
    ...
    console.log(`[GeminiService] Success:`, result);
    return result;
  } catch (err: any) {
    if (err.name === 'AbortError') {
      console.error('[GeminiService] Request timeout');
      throw new Error('Request timeout - AI is taking too long to respond. Please try again.');
    }
    console.error('[GeminiService] Backend call failed:', err);
    throw err;
  }
}
